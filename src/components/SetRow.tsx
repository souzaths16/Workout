...
export function SetHeader({ ex, stage }: { ex: Exercise; stage?: number }) {
  ...
  const loadLabel = ex.loadType === 'pullup_ladder' ? '' : ex.loadType === 'bodyweight' ? t.hoje.assist : t.hoje.load
  return (
...
export function SetRow({ ex, set, stage, onChange, onDone }: { ex: Exercise; set: SetLog; stage?: number; onChange: (p: Partial<SetLog>) => void; onDone: () => void }) {
  ...
  const showLoad = ex.loadType === 'dumbbell_pair' || ex.loadType === 'dumbbell_single' || (ex.loadType === 'bodyweight' && !set.assist)
  return (
    <div className={cn('grid grid-cols-[1.5rem_1fr_1fr_auto_2.5rem] items-center gap-1.5 rounded-md px-0.5 py-1', set.done && 'bg-success/10')}>
      ...
      {showLoad ? (
        ...
        <div className="text-center text-xs text-muted-foreground">{set.assist ? `${t.hoje.assist} ${set.assist}` : ladder && !isHold ? ladder.nome : '—'}</div>
      )}
