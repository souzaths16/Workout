...
import { WEEKDAYS_PT } from '@/domain/dates'
...
export function ChooseDaySheet({ open, onOpenChange, week, excludeDate, onPick }: { open: boolean; onOpenChange: (o: boolean) => void; week: WeekPlan; excludeDate: string; onPick: (date: string) => void }) {
  const options = week.days.map((d, i) => ({ d, i })).filter(({ d }) => d.date !== excludeDate && d.status === 'planned' && TEMPLATE_BY_ID[d.templateId]?.kind === 'lift')
  return (
        ...
        <div className="mt-4 space-y-2">
          ...
          {options.map(({ d, i }) => (
            <button key={d.date} type="button" onClick={() => { onPick(d.date); onOpenChange(false) }}
              ...
              <span><span className="text-muted-foreground">{WEEKDAYS_PT[i]} · </span>{TEMPLATE_BY_ID[d.templateId]?.nome}</span>
            </button>
          ))}
        </div>
      </DialogContent>
