...
const BANDS: BandLevel[] = ['leve', 'media', 'pesada']
    ...
    <div className="p-4 space-y-3">
      ...
      <Card><CardHeader><CardTitle>{t.ajustes.inventory}</CardTitle><CardDescription>Par de halteres até {pairMax} kg cada · halter único até {singleMax} kg</CardDescription></CardHeader>
        <CardContent className="space-y-3">
          ...
          <div><Label>{t.ajustes.bands}</Label><div className="mt-1 flex gap-2">{BANDS.map(b => <Button key={b} size="sm" variant={inv.bands.includes(b) ? 'default' : 'outline'} onClick={() => st.updateInventory({ bands: inv.bands.includes(b) ? inv.bands.filter(x => x !== b) : [...inv.bands, b] })}>{b}</Button>)}</div></div>
      ...
      <Card><CardHeader><CardTitle>{t.ajustes.shopping}</CardTitle></CardHeader>
        <CardContent className="space-y-2 text-sm">
          ...
          <ul className="list-disc pl-5 text-muted-foreground"><li>Anilhas fracionadas de 0,5 kg × 4 que encaixem no cabo do kit (meça o furo; costuma ser 25 mm). Motivo: hoje o menor salto é 2,5 kg por halter, grande demais para elevação lateral e rosca.</li><li>Par de halteres ajustáveis até pelo menos 20 kg cada (tipo dial 2,5–24 kg), ou kettlebells de 16 e 20 kg. Motivo: terra romeno e búlgaro passam de 10 kg por mão cedo; o app avisa quando a prescrição ultrapassar seus pesos.</li><li>Uma banda de assistência grossa, se o seu kit não tiver.</li></ul>
