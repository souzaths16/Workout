...
import { addDays, toISODate, weekdayLabel, weekIndexOf, weekStartOf } from '@/domain/dates'
...
export function Semana() {
  ...
  const st = useStore()
  const [ws, setWs] = useState(() => weekStartOf(st.settings.startDate, today))
  ...
  return (
    <div className="p-4">
      ...
      <div className="space-y-2">
        {week.days.map(d => { const tp = TEMPLATE_BY_ID[d.templateId]; const isToday = d.date === today; return (
          <Card key={d.date} className={cn(isToday && 'border-primary')}>
            <button type="button" className="w-full text-left" onClick={() => setOpen(open === d.date ? null : d.date)}>
              <CardHeader className="flex-row items-center justify-between">
                <div><p className="text-xs text-muted-foreground">{weekdayLabel(d.date)} · {d.date.slice(8)}/{d.date.slice(5, 7)}</p><CardTitle>{tp?.nome ?? d.templateId}</CardTitle>
